...
### 其他告示
1. 现在的文章名称，支持含有洛谷的分类。例如有一篇名为 `t` 的被分在 题解 中的文章，该文章的名称将是 `【题解】t.md`，且文章的最后一行会加上 `分类：题解`。若您想关闭这个功能，可以简单地按下面地方法尝试：
  ![](https://s4.ax1x.com/2022/01/29/H9pLiF.png)
  这是代码最后地截图。通过 `#` 注释掉第一个框内的语句，可以避免文件名出现分类（例如 `【题解】`）的字样；注释掉第二个框内的语句，可以避免文件最后一行加上的分类（例如 `分类：题解`）的字样。
